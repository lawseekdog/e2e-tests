...
from scripts._support.legal_opinion_real_flow_support import (
...
_SUCCESS_STATUSES = {"draft", "review_pending", "approved", "published"}
ANALYSIS_WORK_ORDER: dict[str, Any] = {
    "entry_mode": "analysis",
    "service_type_id": "legal_opinion",
    "delivery_goal": "analysis_only",
    "supporting_document_kinds": [],
}
LEGAL_OPINION_CHAT_RUN: dict[str, Any] = {
...
def _build_start_query(raw_query: str) -> str:
    start_query = _safe_str(raw_query) or DEFAULT_KICKOFF
    if start_query == DEFAULT_KICKOFF:
        return (
            f"{DEFAULT_KICKOFF}\n\n"
...
def _extract_typed_render_state(snapshot: dict[str, Any] | None) -> dict[str, Any]:
    if not isinstance(snapshot, dict):
        return {}
    state = snapshot.get("typed_render_state") if isinstance(snapshot.get("typed_render_state"), dict) else {}
...
def _pick_analysis_auto_action(
    ...
    for row in _extract_runtime_next_actions(snapshot, analysis_projection):
        ...
        if action_type != "focus_blocker":
            continue
        return row
    return {}


def _analysis_auto_focus_blocker_target(action: dict[str, Any] | None) -> str:
...
def _is_auto_answerable_intake_card(card: dict[str, Any] | None) -> bool:
    if not isinstance(card, dict):
        return False
    if _safe_str(card.get("reason_kind")).lower() != "missing_input":
        return False
    if _safe_str(card.get("reason_code")).lower() != "legal_opinion_intake_gate":
        return False
...
def _is_capability_gap_card(card: dict[str, Any] | None) -> bool:
    return (
        ...
        and _safe_str(card.get("reason_code")).lower() == "legal_opinion_capability_gap"
    )
...
def _bundle_quality_report(
    *,
    ...
    typed_render_state: dict[str, Any],
    ...
    legal_opinion_row = (
        artifacts.get("legal_opinion")
        if isinstance(artifacts.get("legal_opinion"), dict)
        else {}
    )
    ...
    if _safe_str(artifact_status).lower() in _SUCCESS_STATUSES:
        score += 5
    ...
    return {
        ...
        "details": {
            ...
            "artifact_status": _safe_str(artifact_status),
...
async def _download_primary_legal_opinion_text(
    ...
    *,
    ...
    artifact_status = _safe_str(row.get("status"))
    ...
    return content, artifact_status
...
async def _collect_round_state(
    *,
    ...
    typed_render_state = _extract_typed_render_state(snapshot)
    ...
    deliverable_text, artifact_status = await _download_primary_legal_opinion_text(
        client,
    ...
    base_bundle_quality = _bundle_quality_report(
        ...
        typed_render_state=typed_render_state,
    ...
    write_json(out_dir / f"{prefix}.typed_render_state.json", typed_render_state)
    ...
    if current_blocker:
        write_json(out_dir / f"{prefix}.current_blocker.json", current_blocker)
    return {
        "matter_id": matter_id,
        "snapshot": snapshot,
...
async def run(args: argparse.Namespace) -> int:
    ...
    start_query = _build_start_query(_safe_str(args.user_query))
    ...
    async with ApiClient(base_url) as client:
        ...
        supervisor.update(status="booting", progress_label="bootstrap.login", next_action="upload_files")
        ...
        supervisor.update(
            status="booting",
            progress_label="bootstrap.upload_files",
        ...
        request_sse = await flow.start_chat_run(
            **LEGAL_OPINION_CHAT_RUN,
            user_query=start_query,
            attachments=uploaded_file_ids,
            max_loops=max(1, int(args.request_max_loops)),
            settle_mode="fire_and_poll",
            label="legal_opinion",
        )
        start_chat_run_counts = event_counts(request_sse if isinstance(request_sse, dict) else {})
        write_json(out_dir / "00.start_chat_run.sse.json", request_sse if isinstance(request_sse, dict) else {})
        supervisor.update(
            status="running",
            ...
            extra={"start_chat_run_event_counts": start_chat_run_counts},
        )

        start_chat_run_blocker = await flow.actionable_card_from_sse(request_sse if isinstance(request_sse, dict) else {})
        if _is_capability_gap_card(start_chat_run_blocker):
            if not _capability_gap_card_matches_overrides(start_chat_run_blocker, FLOW_OVERRIDES):
                await flow.refresh()
                ...
                summary = {
                    ...
                    "current_blocker": gap_round["current_blocker"] or start_chat_run_blocker,
                ...
                supervisor.update(
                    status="blocked",
                    progress_label="terminal.capability_gap",
                    session_id=session_id,
                    matter_id=_safe_str(flow.matter_id),
                    ...
                    blocker_card=gap_round["current_blocker"] if isinstance(gap_round.get("current_blocker"), dict) else start_chat_run_blocker,
                    current_blocker={"type": "blocked", "summary": "capability_gap"},
        ...
        def _analysis_round_ready(round_state: dict[str, Any]) -> bool:
            pending = round_state["current_blocker"] if isinstance(round_state.get("current_blocker"), dict) else {}
        ...
        for step_no in range(1, max(1, int(args.max_steps)) + 1):
            ...
            if current_progress_token != analysis_progress_token:
                ...
                analysis_action_cooldown = 0
            analysis_pending_card = analysis_round["current_blocker"] if isinstance(analysis_round.get("current_blocker"), dict) else {}
            ...
            supervisor.update(
                ...
                progress_label="poll.analysis_ready",
                session_id=session_id,
                matter_id=_safe_str(flow.matter_id),
                ...
                blocker_card=analysis_pending_card,
                current_blocker=(
                    analysis_pending_card
                    if isinstance(analysis_pending_card, dict) and analysis_pending_card
                    else ({"type": "blocked", "summary": "analysis_not_ready"} if not analysis_ready else None)
                ),
            ...
            current_blocker = analysis_pending_card
            if _is_auto_answerable_intake_card(current_blocker):
                summary = {
                ...
                supervisor.update(
                    status="blocked",
                    progress_label="terminal.unexpected_intake_card",
                    session_id=session_id,
                    matter_id=_safe_str(flow.matter_id),
                    snapshot=analysis_round["snapshot"] if isinstance(analysis_round.get("snapshot"), dict) else {},
                    execution_snapshot=analysis_execution_snapshot,
                    ...
                    current_blocker={"type": "blocked", "summary": "unexpected_intake_card"},
                    next_action="inspect_summary",
                    artifact_refs={"summary": str(out_dir / "summary.json")},
                )
            ...
            if auto_action and analysis_action_cooldown <= 0 and _analysis_allows_auto_review_card(analysis_snapshot):
                ...
                target = _analysis_auto_focus_blocker_target(auto_action)
                if target:
                    sse = await flow.step()
            ...
            supervisor.update(
                ...
                current_blocker={"type": "blocked", "summary": "legal_opinion_analysis_not_ready"},
        ...
        if _is_capability_gap_card(analysis_round["current_blocker"]):
            summary = {
                "status": "capability_gap",
                "current_blocker": analysis_round["current_blocker"],
                "flow_scores": analysis_round["flow_scores"],
                "bundle_quality": analysis_round["bundle_quality"],
            }
            write_json(out_dir / "summary.json", summary)
            supervisor.update(
                ...
                execution_traces=analysis_round["execution_traces"] if isinstance(analysis_round.get("execution_traces"), list) else None,
                blocker_card=analysis_round["current_blocker"] if isinstance(analysis_round.get("current_blocker"), dict) else None,
                current_blocker={"type": "blocked", "summary": "capability_gap"},
                next_action="inspect_summary",
                artifact_refs={"summary": str(out_dir / "summary.json")},
            )
        ...
        ready_pending_card = (
            analysis_round["current_blocker"]
            if isinstance(analysis_round.get("current_blocker"), dict)
            else {}
        )
        summary = {
            "base_url": base_url,
            ...
            "uploaded_file_ids": uploaded_file_ids,
            "start_chat_run_event_counts": start_chat_run_counts,
            ...
            "typed_render_state": final_round["typed_render_state"],
            "current_blocker": ready_pending_card,
            "execution_status": _safe_str((final_round.get("execution_snapshot") or {}).get("status")),
            "execution_phase_id": _current_execution_phase_id(final_round.get("execution_snapshot")),
            "success": True,
        }
        write_json(out_dir / "summary.json", summary)
        write_json(
        ...
        supervisor.update(
            ...
            blocker_card=ready_pending_card,
            next_action="inspect_summary",
            artifact_refs={"summary": str(out_dir / "summary.json")},
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--user-query", default=DEFAULT_KICKOFF, help="Initial unified request query")
    ...
    parser.add_argument("--action-max-loops", type=int, default=24, help="chat run action max_loops")
